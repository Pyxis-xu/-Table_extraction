...
def write_text_to_excel(worksheet, row, col, text, merge_format):
    worksheet.write(row, col, text, merge_format)
...
def main():
    ...
    workbook = xlsxwriter.Workbook('extract1.xlsx')
    ...
    for i in range(len(lt_list_x)):
        for j in range(len(lt_list_y)):
            d['cell{}_{}'.format(i, j)] = cell([lt_list_x[i], lt_list_y[j]], [rd_list_x[i], rd_list_y[j]],
                                               [lt_list_x[i], lt_list_y[j]])

    for i in range(len(lt_list_x)):
        for j in range(len(lt_list_y)):
            p1 = [d['cell{}_{}'.format(i, j)].rd[1], d['cell{}_{}'.format(i, j)].lt[0]]
            p2 = [d['cell{}_{}'.format(i, j)].rd[1], d['cell{}_{}'.format(i, j)].rd[0]]
            p3 = [d['cell{}_{}'.format(i, j)].lt[1], d['cell{}_{}'.format(i, j)].rd[0]]
    ...
    for i in range(len(lt_list_x)):
        for j in range(len(lt_list_y)):
            crop_list['{},{}'.format(d['cell{}_{}'.format(i, j)].belong[0], d['cell{}_{}'.format(i, j)].belong[1])] = \
            d['cell{}_{}'.format(i, j)].rd
    ...
    for key in crop_list.keys():
        ...
        if lt_col == rd_col and lt_row == rd_row:
            worksheet.write('{}{}'.format(lt_col, lt_row), None, merge_format)
        else:
            worksheet.merge_range('{}{}:{}{}'.format(lt_col, lt_row, rd_col, rd_row), None, merge_format)

        # 读取图片中的文字并写入对应的单元格
        img_path = 'crop/{}.jpg'.format(key)
        text = pytesseract.image_to_string(img_path, lang='eng')
        write_text_to_excel(worksheet, lt_row-1, ord(lt_col)-ord('A'), text, merge_format)

        cell_range = '{}{}:{}{}'.format(lt_col, lt_row, rd_col, rd_row)
        worksheet.conditional_format(cell_range, {'type': 'no_errors', 'format': merge_format})
